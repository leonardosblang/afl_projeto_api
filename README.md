...
Há um script que valida o backend pra cada imagem do Figma. Para rodar:
```sh
cd figma_examples
...
Há um script adicional chamado `company_endpoint_test.py` que testa se uma empresa pode ter n contratos e também a exclusão de uma empresa e verifica se os contratos relacionados são também excluídos(está na mesma pasta que o anterior).
Para rodar:
```sh
...
Os testes unitários para validar cada funcionalidade da aplicação estão localizados na pasta `tests`. Para rodar os testes:
```sh
cd tests
